...
colscale <-  c("#a6cee3","#1f78b4","#b2df8a","#33a02c","#fb9a99","#e31a1c","#fdbf6f","#ff7f00","#cab2d6","#6a3d9a","#b15928")
names(colscale) <- c("Colombia", "FrenchGuiana", "Gabon", "Ghana", "Iran", "Peru", "PNG", "Thailand", "Uganda", "Venezuela", "Ape")
...
ggplot(avg_proportions, aes(x=Isolate, y=reWeighted, fill=Country)) + 
  ...
  theme(text = element_text(size=15)) + fillScale
```
...
#Proportion file
unc_avg_proportions <- data.frame()
for (f in prop_files){
  proportion <- fread(f, data.table = FALSE)
  colnames(proportion) <- c("Target", "Reference", "Proportion")
  prop_merge <- merge(proportion, isolateInformation, by.x="Reference", by.y = "Isolate", all.x=TRUE)
  
  sum_location <- purrr::map_dfr(1:100, function(i){
    temp <- prop_merge %>% group_by(Country) %>%
      summarise(
        num.isolates = n(),
        sum.prop = sum(sample(Proportion, 10)),
        avg.proportion = mean(sample(Proportion, 10))
      )
    temp$reWeighted <- temp$avg.proportion/sum(temp$avg.proportion)
    return(temp)
  })
  ...
  iso <- gsub("_proportions.txt","",iso)
  sum_location$Isolate <- rep(iso, nrow(sum_location))
  unc_avg_proportions <- rbind(unc_avg_proportions, sum_location)
}

colnames(isolateInformation) <- c("Isolate","Origin")

unc_avg_prop_w_iso <- merge(unc_avg_proportions, isolateInformation, by.x="Isolate", by.y="Isolate", all.x=TRUE)

summary_prop_table <- unc_avg_prop_w_iso %>% group_by_(.dots=c("Country", "Origin")) %>%
  summarise(
    mean=mean(reWeighted, na.rm=TRUE),
    sd=sd(reWeighted, na.rm=TRUE)
  )
...
ggplot(summary_prop_table, aes(Country, Origin, fill = mean)) + geom_raster() +
  scale_fill_gradientn(
                       colours = c('#b2182b','#ef8a62','#fddbc7','#d1e5f0','#67a9cf','#2166ac')) +
  geom_text(data=summary_prop_table,aes(x=Country, y=Origin
...
```{r}
model.data.df <- fread("./data/isolate_information.csv"
                            , header=TRUE
                            , data.table = FALSE)
#Add in country information
model.data.df$Country <- unlist(lapply(model.data.df$Location
                                            , function(x) {
                                              str_split(x,  "_", n=2)[[1]][[1]]}))

#Remove duplicate entries of isolates that have been sequenced more than once
model.data.df <- model.data.df[!duplicated(model.data.df$Isolate),]
model.data.df$Disease_Status[model.data.df$Disease_Status==""] <- "Uncomplicated"

temp.df <- dcast(avg_proportions, Isolate ~ Country, value.var = "sum.prop")
model.data.df <- merge(model.data.df, temp.df, by.x="Isolate", by.y = "Isolate", all.y = TRUE)

#get number of types per isolate
...
n.types <- colSums(otuMatrix)

model.data.df$n.types <- n.types[match(model.data.df$Isolate,names(n.types))]

library(nnet)
test <- multinom(Country ~ Uganda + Ghana + Gabon + Iran + Thailand + PNG + 
                   Peru + FrenchGuiana + Venezuela + Colombia + Disease_Status + n.types, 
                 data = model.data.df)
summary(test)
...
This indicates that the overiding signal is due to the isolates country of origin with the number of types having a significant but very small effect on the propotions found to have originated from Ghana. 

```{r}
colombia_props <- avg_proportions[avg_proportions$Origin=="Colombia",]
colombia_props <- colombia_props %>% group_by(Isolate) %>% summarise(
  african.prop = sum(reWeighted[Country %in% c("Uganda", "Gabon", "Ghana")]),
  s.american = sum(reWeighted[Country %in% c("FrenchGuiana", "Peru", "Venezuela")])
)

wilcox.test(colombia_props$african.prop,
            colombia_props$s.american,
            paired = TRUE)
```
...
gg <- gg + geom_bar(stat = "identity")
gg <- gg + theme_bw() + theme(axis.text.x = element_text(angle = 90)) + fillScale
gg
```
...
summ_location$reWeighted <- summ_location$avg.proportion/sum(summ_location$avg.proportion)*100
...
rask_block_aln = data.frame(block=c(79,54,171,161,131,64,210,14,5,60,36,153,88,163),
left=c(33,33,33,269,118,232,232,113,354,515,460,704,701,701),
right=c(126,118,50,344,344,328,344,269,433,730,709,730,730,730), stringsAsFactors = FALSE)

gg_block_aln <- ggplot(rask_block_aln, aes(y=factor(block), yend=factor(block), x=left, xend=right)) +
  geom_segment(size=2) +
  theme_bw() + ylab("Homology Block") + xlab("Alignment Position") +
...
plot_data <- data.frame(position=1:length(recombination)
                        ...
                        , stringsAsFactors = FALSE)
window_size <- 15
windows <- lapply(1:(nrow(plot_data)-window_size), function(i) 1:window_size+i)
avg_recomb_window <- unlist(lapply(windows, function(index) (sum(plot_data$recombination[index])/sum(plot_data$occupancy[index]))/15))
q <- quantile(avg_recomb_window, c(0.25,0.75))
outliers <- avg_recomb_window>(q[[2]]+1.5*(q[[2]]-q[[1]]))
outliers <- unique(unlist(windows[outliers]))
plot_data$outlier <- (1:nrow(plot_data) %in% outliers) & (plot_data$occupancy>0.2) & (plot_data$recombination>100)

plot_data <- melt(plot_data, id.vars = c("position", "outlier"))
plot_data$variable <- as.character(plot_data$variable)
plot_data$variable[plot_data$variable=="recombination"] <- "recombination count"
plot_data$variable[plot_data$variable=="occupancy"] <- "occupancy of multiple sequence alignment columns"
gg <- ggplot(plot_data, aes(x=position, y=value, col=outlier), fill=variable)
gg <- gg + geom_bar(stat='identity')
gg <- gg + facet_wrap(~variable, scales = "free_y", ncol=1)
gg <- gg + theme_bw() + xlab("") + theme(text = element_text(size=14))
gg <- gg + ylab("")
gg + gg_block_aln + patchwork::plot_layout(ncol=1)
```
