...
rownames(majorTypeMatrix) <- paste(annotation_row$shortID, annotation_row$Countries, sep="-")
...
map <- data.frame(chromosome=rep(1,ncol(otuMatrixNoLab_t)),
                  id=paste("rs", 1:ncol(otuMatrixNoLab_t),sep=""),
                  dist=rep(0,ncol(otuMatrixNoLab_t)),
                  loc=1:ncol(otuMatrixNoLab_t),
                  stringsAsFactors = FALSE)
...
The small dip in the cross-validation error at K=6x appears to correspond with Admixture infering different hidden populations for the three African countries. For all K PNG does is not seperated from the African background cluster. This could be because of the different experimental method used in obtaining the PNG isolates.
...
word_usage <- fread("./processed_data/ffp_word_usage.txt",
                    data.table = FALSE)
word_usage$V1[which.max(word_usage$V2)]
plot(word_usage)
```
...
#Save a list of the isolates with at least 20 types.
write.table(colnames(otuMatrixNoLab), file="./ffp_data/keep_isolates.txt"
            , quote = FALSE, sep=",", col.names = FALSE, row.names = FALSE)
```

Thus a choice of l=20 appears to be appropriate.

We would like to trim the PNG reads to ensure they are from a similar region.
```{bash, eval=FALSE}
hmmalign -o ./ffp_data/hmmalign_dbla_combined_all_454_noPOR.sto ./data/atag.hmm ./data/combined_all_454_noPOR.fasta
hmmbuild ./ffp_data/dbla_combined_all_454_noPOR.hmm ./ffp_data/hmmalign_dbla_combined_all_454_noPOR.sto
hmmalign --trim -o ./ffp_data/hmmalign_dbla_combined_all_454_noPOR_VS_combined_454_tessema.sto ./ffp_data/dbla_combined_all_454_noPOR.hmm ./processed_data/combined_454_tessema.fas
esl-reformat fasta ./ffp_data/hmmalign_dbla_combined_all_454_noPOR_VS_combined_454_tessema.sto > ./ffp_data/combined_454_tessema_trimmed.fasta
```

We now want to get the set of sequences (excluding the labratory isolates) from which to cluster.
```{python, eval=FALSE}
from mungo.fasta import FastaReader
keep=set()
for line in open("./ffp_data/keep_isolates.txt", 'rU'):
  keep.add(line.strip())

with open("./ffp_data/combined_454_tessema_trimmed_keepIsos.fasta", 'w') as outfile:
  for h,s in FastaReader("./ffp_data/combined_454_tessema_trimmed.fasta"):
...
```{bash, eval=FALSE}
python ./scripts/ffp.py --kmer_length 20 --out ./processed_data/ffp_distance_matrix.phylip --seq ./ffp_data/combined_454_tessema_trimmed_keepIsos.fasta --verbose
```
